<nav id="main-header" class="souk-navbar fixed top-0 left-0 right-0 z-50 w-full">
    <div id="navbar-shell"
        ...
        <div class="max-w-7xl mx-auto px-4 py-3 md:px-6 lg:px-8">
            <div class="flex items-center justify-between gap-4">
                ...
                <form action="{{ route('public.search') }}" method="GET"
                    ...
                    <div
                        class="search-pill flex items-stretch w-full rounded-full bg-slate-100 dark:bg-slate-800 shadow-sm overflow-hidden">
                        ...
                        <div
                            ...
                            <input type="text" name="q" value="{{ request('q') }}"
                                placeholder="{{ __('website.navbar.searchPlaceholder') }}"
                                class="w-full ltr:pl-11 rtl:pr-11 ltr:pr-4 rtl:pl-4 py-2 bg-slate-100 dark:bg-transparent text-sm font-medium text-slate-700 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none ltr:rounded-l-full rtl:rounded-r-full">
                        </div>

                        <!-- Middle Segment — Keyword Search Button -->
                        <button type="submit" name="search_mode" value="keyword"
                            class="px-5 flex items-center justify-center rounded-none font-bold text-xs uppercase tracking-wider text-slate-700 dark:text-slate-200 bg-white hover:bg-slate-50 dark:bg-slate-700 dark:hover:bg-slate-950 transition-colors border-x border-slate-300/50 dark:border-slate-800">
                            {{ __('website.navbar.search') }}
                        </button>

                        <!-- Right Segment — AI Button -->
                        <button type="submit" name="search_mode" value="semantic"
                            class="relative shrink-0 flex items-center gap-1.5 px-5 ltr:rounded-r-full ltr:rounded-l-none rtl:rounded-l-full rtl:rounded-r-none bg-gradient-to-r from-indigo-500 to-purple-500 text-[11px] font-extrabold uppercase tracking-wider text-white hover:shadow-lg hover:shadow-indigo-500/30 hover:scale-[1.02] transition-all duration-300 overflow-hidden group/ai">
                ...
                <div class="flex items-center gap-3 md:gap-4">
                    ...
                    <div
                        class="flex items-center gap-1.5 px-2 py-1.5 bg-slate-100/70 dark:bg-slate-800/50 border border-slate-200/50 dark:border-slate-700/30 rounded-full backdrop-blur-sm">
                        <!-- Theme Toggle -->
                        ...
                        <div class="relative group">
                            <button
                                class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-black uppercase tracking-wider text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                {{ app()->getLocale() }}
                            ...
                            <div
                                class="absolute top-full right-0 mt-2 w-36 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 rounded-3xl p-1.5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all translate-y-2 group-hover:translate-y-0 shadow-xl shadow-black/5 z-50">
                                @foreach(['en' => 'English', 'fr' => 'Français', 'ar' => 'العربية'] as $code => $name)
                                    <a href="{{ route('lang.switch', $code) }}"
                                    class="flex items-center justify-between px-3 py-2.5 rounded-full text-[11px] font-bold hover:bg-indigo-50 dark:hover:bg-indigo-950/30 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors {{ app()->getLocale() == $code ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-950/20' : 'text-slate-600 dark:text-slate-300' }}">
                                        {{ $name }}
                    ...
                    @auth
                        <div class="relative group">
                            <button
                                class="flex items-center gap-2.5 px-3.5 py-2.5 bg-gradient-to-br from-indigo-500/10 to-purple-500/10 dark:from-indigo-500/20 dark:to-purple-500/20 border border-indigo-200/40 dark:border-indigo-400/20 rounded-full hover:shadow-md hover:shadow-indigo-500/10 transition-all">
                                <div
